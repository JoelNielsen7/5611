...
PVector wind_speed = new PVector(0, 0, 1);
...
void draw() {
  ...
  for (int i = 0; i < numX-1; i++){
    for(int j = 0; j < numY-1; j++){
      ...
      //vertex(Pos[i+1][j].x, Pos[i+1][j].y, Pos[i+1][j].z, 0, 1);
      //vertex(Pos[i][j+1].x, Pos[i][j+1].y, Pos[i][j+1].z, 1, 1);
      //endShape();
      //beginShape();
      //texture(img);
      //vertex(Pos[i+1][j+1].x, Pos[i+1][j+1].y, Pos[i+1][j+1].z, 0, 0);
      //vertex(Pos[i+1][j].x, Pos[i+1][j].y, Pos[i+1][j].z, 1, 0);
      //vertex(Pos[i][j+1].x, Pos[i][j+1].y, Pos[i][j+1].z, 1, 1);
      float a = float(i)/float(numX-1);
      float b =float(j)/float(numY-1);
      float c = float(j+1)/float(numY-1);
      float d = float(i+1)/float(numX-1);
      //println(float(i)/float(numX-1));
      //print(a, b, c, d);
      texture(img);
      vertex(Pos[i][j].x, Pos[i][j].y, Pos[i][j].z, a, b);
      vertex(Pos[i+1][j].x, Pos[i+1][j].y, Pos[i+1][j].z, d, b);
      vertex(Pos[i][j+1].x, Pos[i][j+1].y, Pos[i][j+1].z, a, c);
      endShape();
      beginShape();
      texture(img);
      vertex(Pos[i][j+1].x, Pos[i][j+1].y, Pos[i][j+1].z, a, c);
      vertex(Pos[i+1][j].x, Pos[i+1][j].y, Pos[i+1][j].z, d, b);
      vertex(Pos[i+1][j+1].x, Pos[i+1][j+1].y, Pos[i+1][j+1].z, d, c);
      endShape();
    }
  }
  //drawSphere();
